...
# Added Additional Properties and Methods for UDPStream by heyqule

class Aurora:
    def __init__(self, ip_address: str, port: int, auth_token: str):
        self.baseUrl = f"http://{ip_address}:{port}/api/v1/{auth_token}/"
        self.ip_address = ip_address
        self.auth_token = auth_token
        self.port = port

    def __repr__(self):
        return f"<Aurora({self.ip_address} @ {self.port})>"
    ...
    def panel_ids(self):
        panel_ids = []
        position_data = self.panel_positions
        for panel in position_data['positionData']:
            panel_ids.append(panel['panelId'])
